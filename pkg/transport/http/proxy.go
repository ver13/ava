...
func getURL(addr string) (*url.URL, *errorAVA.Error) {
	r := &http.Request{
		URL: &url.URL{
			Scheme: "https",
			Host:   addr,
...
func (p *pbuffer) Read(b []byte) (int, *errorAVA.Error) {
	return p.r.Read(b)
}

func proxyDial(conn net.Conn, addr string, proxyURL *url.URL) (_ net.Conn, err *errorAVA.Error) {
	defer func() {
		if err != nil {
			conn.Close()
		}
...
func newConn(dial func(string) (net.Conn, *errorAVA.Error)) func(string) (net.Conn, *errorAVA.Error) {
	return func(addr string) (net.Conn, *errorAVA.Error) {
		// get the proxy url
