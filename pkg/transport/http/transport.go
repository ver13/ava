...
import (
	...
	mls "github.com/ver13/ava/pkg/common/crypto"
	errorAVA "github.com/ver13/ava/pkg/common/error"
	maddr "github.com/ver13/ava/pkg/common/utils/addr"
	"github.com/ver13/ava/pkg/common/utils/buf"
	mnet "github.com/ver13/ava/pkg/common/utils/net"
	transportAVA "github.com/ver13/ava/pkg/transport"
)

type httpTransport struct {
	opts transportAVA.Options
}
...
type httpTransportClient struct {
	...
	addr     string
	conn     net.Conn
	dialOpts transportAVA.DialOptions
	once     sync.Once
...
func (h *httpTransportClient) Send(m *transportAVA.Message) *errorAVA.Error {
...
func (h *httpTransportClient) Recv(m *transportAVA.Message) *errorAVA.Error {
	if m == nil {
...
func (h *httpTransportClient) Close() *errorAVA.Error {
	h.once.Do(func() {
		h.Lock()
		h.buff.Reset(nil)
		h.Unlock()
...
func (h *httpTransportSocket) Recv(m *transportAVA.Message) *errorAVA.Error {
	if m == nil {
		return errors.New("message passed in is nil")
	}
	if m.Header == nil {
...
func (h *httpTransportSocket) Send(m *transportAVA.Message) *errorAVA.Error {
	if h.r.ProtoMajor == 1 {
		// make copy of header
...
func (h *httpTransportSocket) error(m *transportAVA.Message) *errorAVA.Error {
	if h.r.ProtoMajor == 1 {
		rsp := &http.Response{
			Header:        make(http.Header),
			Body:          ioutil.NopCloser(bytes.NewReader(m.Body)),
...
func (h *httpTransportSocket) Close() *errorAVA.Error {
	h.mtx.Lock()
	defer h.mtx.Unlock()
	select {
	case <-h.closed:
...
func (h *httpTransportListener) Close() *errorAVA.Error {
	return h.listener.Close()
}

func (h *httpTransportListener) Accept(fn func(transportAVA.SocketI)) *errorAVA.Error {
	// create handler mux
...
func (h *httpTransport) Dial(addr string, opts ...transportAVA.DialOption) (transportAVA.ClientI, *errorAVA.Error) {
	dopts := transportAVA.DialOptions{
		Timeout: transportAVA.DefaultDialTimeout,
	}
	...
	if h.opts.Secure || h.opts.TLSConfig != nil {
		...
		conn, err = newConn(func(addr string) (net.Conn, *errorAVA.Error) {
...
func (h *httpTransport) Listen(addr string, opts ...transportAVA.ListenOption) (transportAVA.ListenerI, *errorAVA.Error) {
	var options transportAVA.ListenOptions
	for _, o := range opts {
		o(&options)
	}
	...
	if h.opts.Secure || h.opts.TLSConfig != nil {
		...
		fn := func(addr string) (net.Listener, *errorAVA.Error) {
			if config == nil {
				hosts := []string{addr}
				...
				cert, err := mls.Certificate("ver13", hosts...)
				if err != nil {
					return nil, err
				}
...
func (h *httpTransport) Init(opts ...transportAVA.Option) *errorAVA.Error {
	for _, o := range opts {
		o(&h.opts)
	}
	return nil
...
func (h *httpTransport) Options() transportAVA.Options {
	return h.opts
}
